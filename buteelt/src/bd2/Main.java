...
public class Main {
	...
	//1989 - 1993 
	 ...
	 private static long totalWeeks(int startYear) {
	        LocalDate startDate = LocalDate.of(startYear, 9, 1);
	        LocalDate endDate = LocalDate.of(startYear + 1, 5, 31);

	        // Adjust start date to the first Monday on or after September 1
	        if (startDate.getDayOfWeek() != DayOfWeek.MONDAY) {
	            startDate = startDate.with(TemporalAdjusters.next(DayOfWeek.MONDAY));
	        }
	        
	        // Adjust end date to the last Sunday on or before May 31
	        if (endDate.getDayOfWeek() != DayOfWeek.SUNDAY) {
	            endDate = endDate.with(TemporalAdjusters.previous(DayOfWeek.SUNDAY));
	        }

	        // Include partial weeks if they round up to a full week when combined
	        long days = ChronoUnit.DAYS.between(startDate, endDate) + 1; // +1 to include the last day
	        long weeks = (days + 6) / 7; // This will round up to include partial weeks
	 ...
	 public static void main(String[] args) {
            ...
            int startYear = 1978 ; 
 	        ...
 	        System.out.println("------------------------------------------------------------------------------------------------------------------------------------");
	        System.out.printf("| %8s | %-5s | %-4s | %-8s | %-9s | %-7s | %-15s | %-10s | %-12s |%-13s |%-8s | %-15s |%n",
	        		
	                          "Он", "Статус", "Анги", "Амралт", "Хичээл", "Ажлын ", "Хичээлийн цаг", "1 хичээл", "7 хоногт", "7 хоногт(цаг)","Дадлага","бүтэн 7 хоног","");
	        System.out.println("------------------------------------------------------------------------------------------------------------------------------------");
	        ...
	        // Outer loop for rows
	                ...
	                System.out.printf("| %8s| %-5s | %-4s | %-8s | %-9s | %-7s | %-15s | %-10s | %-12s |%-13s |%-8s | %-15s |%n",
	                                  a +"-"+ nextYear ,  Oyutan1988(a) , angi(a) , Amralt(a , nextYear)+" хоног", WorkDay1(a , nextYear)+" хоног", 6 +"өдөр", hicheelTsag(a)+"(7 хоногт)", hicheelminut(a)+" (минут)",Integer.parseInt( hicheelTsag(a))* Integer.parseInt(hicheelminut(a))+" (минут)",( Integer.parseInt( hicheelTsag(a))* Integer.parseInt(hicheelminut(a)))/60+" (цаг)",dadlaga(a),totalWeeks(a));
	            	}
	    	        System.out.println("------------------------------------------------------------------------------------------------------------------------------------");
...
// Outer loop for rows
       ...
       System.out.printf("| %8s| %-5s | %-4s | %-8s | %-9s | %-7s | %-15s | %-10s | %-12s |%-13s |%-8s | %-15s |%n",
                         a +"-"+ nextYear1 ,  Oyutan2018(a) , angi1(a) , Amralt1(a , nextYear1)+" хоног", WorkDay(a , nextYear1)+" хоног", 5 +"өдөр", hicheelTsag1(a)+"(7 хоногт)", hicheelminut1(a)+" (минут)",Integer.parseInt( hicheelTsag1(a))* Integer.parseInt(hicheelminut1(a))+" (минут)",( Integer.parseInt( hicheelTsag1(a))* Integer.parseInt(hicheelminut1(a)))/60+" (цаг)",dadlaga1(a),totalWeeks(a));
   	}
       System.out.println("------------------------------------------------------------------------------------------------------------------------------------");   
       System.out.println("1р асуулт");
